...
const checkFields = () => {
  return Promise.try(() => {
    if (!$('#passphrase').val()) {
      throw new Error('Please enter your wallet password in the "Wallet Password" field')
    }
    if (!$('#boxa').val()) {
      throw new Error('Please enter your User Key')
    }
    if (!$('#boxb').val()) {
      throw new Error('Please enter your Backup Key')
    }
    if (!$('#boxc').val()) {
      throw new Error('Please enter your Bitwala Public Key')
    }
...
const sendTransaction = () => {
  ...
  if (userMessage !== 'I agree to send ' + SEND_AMOUNT + ' BCH with a fee of ' + FEE_AMOUNT + ' BCH to address ' + RECEIVE_ADDRESS) {
    ...
    axios.post('https://blockdozer.com/insight-api/tx/send', {rawtx: TRANSACTION_HEX})
    .then(response => {
      ...
      updateConsole("Thank you for using the Bitwala recovery tool by CONDACORE!\n")
